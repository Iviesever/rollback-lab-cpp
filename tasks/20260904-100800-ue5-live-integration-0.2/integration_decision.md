...
Raw evidence: `artifacts/ue5-0.2/spike/`. All three manifests contain plugin products; the Engine game executable timestamps remain the installed July build. UAT log folders and configuration cache are redirected to repository paths. UE 5.8 always uses its UBA executor even with -NoUBA and emitted a default user-cache trace outside the repository during discovery. This containment deviation is recorded; supported redirection is still under investigation and must be resolved before the final UE evidence gate. The spike proves DLL staging, not completed runtime/package acceptance.
