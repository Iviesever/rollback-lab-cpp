...
13. **UDP profile and parity.** Reproduce an advertised ABI-profile mismatch and prove no gameplay frame advances before agreement. Explain why the digest binds ABI/protocol/simulation/seeds/target while keeping Protocol v1 unchanged. Run a normal pair, verify confirmed hash/replay and aggregate correction, and explain why UDP report counters need not equal the seeded sample or each other.
14. **Packaged and UDP failure diagnosis.** On a disposable package copy, alter a DLL, screenshot or nested report and show exact rejection. Then run an expected UDP missing-peer/watchdog case, verify its typed context, provisional failure summary and complete owned-child teardown. Explain why null hashes are acceptable only in partial failure diagnostics, why Normal requires complete evidence, and why Editor success cannot establish this deployment result.
